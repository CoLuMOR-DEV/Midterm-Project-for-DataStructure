...
class LinkedListStackGUI(tk.Tk):
    ...
    def _create_linkedlist_widgets(self, parent):
        ...
        # --- CHANGES START HERE ---
        # Added ipady=4 to make buttons taller and pady=4 for vertical spacing.
        
        ttk.Button(ll_frame, text="Append", command=None, style="Dark.TButton").grid(row=2, column=0, padx=(0, 2), pady=4, sticky="ew", ipady=4)
        ttk.Button(ll_frame, text="Prepend", command=None, style="Dark.TButton").grid(row=2, column=1, padx=(2, 0), pady=4, sticky="ew", ipady=4)
        
        # Added padx=5 for horizontal margins on full-width buttons.
        ttk.Button(ll_frame, text="Random nodes", command=None, style="Dark.TButton").grid(row=3, column=0, columnspan=2, padx=5, pady=4, sticky="ew", ipady=4)
        ttk.Button(ll_frame, text="Recursive traverse", command=None, style="Dark.TButton").grid(row=4, column=0, columnspan=2, padx=5, pady=4, sticky="ew", ipady=4)
        ttk.Button(ll_frame, text="Iterative traverse (use stack)", command=None, style="Dark.TButton").grid(row=5, column=0, columnspan=2, padx=5, pady=4, sticky="ew", ipady=4)
        ttk.Button(ll_frame, text="Recursive reverse", command=None, style="Dark.TButton").grid(row=6, column=0, columnspan=2, padx=5, pady=4, sticky="ew", ipady=4)
        ttk.Button(ll_frame, text="Clear linked list", command=None, style="Dark.TButton").grid(row=7, column=0, columnspan=2, padx=5, pady=4, sticky="ew", ipady=4)
        # --- CHANGES END HERE ---
        
        ttk.Button(parent, text="Refresh Visualization", command=None, style="Dark.TButton").grid(row=1, column=0, padx=10, pady=15, sticky="ew", ipady=12)
